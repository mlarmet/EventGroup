  ...
  public void clickOk() {
    ...
    if (nom.trim().isEmpty() || prenom.trim().isEmpty() || pseudo.trim().isEmpty()
      ...
      if (this.pseudo != null) {
        ...
        GestionEvenements.modifierNomMembre(GestionEvenements.getMembre(this.pseudo), nom);
        GestionEvenements.modifierPrenomMembre(GestionEvenements.getMembre(this.pseudo), prenom);
        GestionEvenements.modifierAdresseMailMembre(GestionEvenements.getMembre(this.pseudo),
            email);
        ...
        if (!this.pseudo.equalsIgnoreCase(pseudoInput)) {
          if (GestionEvenements.modifierPseudoMembre(GestionEvenements.getMembre(this.pseudo),
              pseudoInput) == 0) {
            Alert alert = new Alert(AlertType.ERROR);
            alert.setTitle("Erreur pseudo");
            alert.setHeaderText("Pseudo déjà utilisé");
            alert.setContentText("Ce pseudo est déjà utilisé par un autre utilisateur");
